describe('interface', () => {
    ...
    describe('set theme', () => {
        ...
        it('accepts all supported themes', () => {
            const sut = document.createElement('os-window');
            try {
                for(let i = 0; i < sut.supportedThemes.length; i++) {
                    sut.theme = sut.supportedThemes[i];
                }
            } catch (e) {
                chai.expect.fail(e);
            }
    ...
    describe('get theme', () => {
        it('returns the default theme when the attribute theme is null', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.theme).to.equal(sut.defaultTheme);
        });

        it('returns the value of the attribute theme for supported themes', () => {
            const sut = document.createElement('os-window');
            for(let i = 0; i < sut.supportedThemes.length; i++) {
                sut.setAttribute('theme', sut.supportedThemes[i]);
                chai.expect(sut.theme).to.equal(sut.supportedThemes[i]);
            }
        });
    ...
    describe('get supportedThemes', () => {
        it('returns an array of all supported themes', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.supportedThemes).to.be.a('Array');
            chai.expect(sut.supportedThemes).to.have.lengthOf(2);
            chai.expect(sut.supportedThemes).to.include.members(['light', 'dark']);
        });
    });
    ...
    describe('get defaultTheme', () => {
        it('returns a supported theme', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.defaultTheme).to.be.oneOf(sut.supportedThemes);
        });

        it('returns light', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.defaultTheme).to.be.equal('light');
        });
    });
    ...
    describe('set osTheme', () => {
        ...
        it('does not change theme for unsupported values', () => {
            const sut = document.createElement('os-window');
            const wrongOsTheme = 'fail';
            const expectedTheme = sut.osTheme;
            try {
                sut.osTheme = wrongTheme;
            } catch (e) { }
            finally {
        ...
        it('accepts all supported os-themes', () => {
            const sut = document.createElement('os-window');
            try {
                for(let i = 0; i < sut.supportedOsThemes.length; i++) {
                    sut.osTheme = sut.supportedOsThemes[i];
                }
            } catch (e) {
                chai.expect.fail(e);
            }
    ...
    describe('get osTheme', () => {
        it('returns the default os theme when the attribute os-theme is null', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.osTheme).to.equal(sut.defaultOsTheme);
        });

        it('returns the value of the attribute os-theme for supported os themes', () => {
            const sut = document.createElement('os-window');
            for(let i = 0; i < sut.supportedOsThemes.length; i++) {
                sut.setAttribute('os-theme', sut.supportedOsThemes[i]);
                chai.expect(sut.osTheme).to.equal(sut.supportedOsThemes[i]);
            }
        });
    ...
    describe('get supportedOsThemes', () => {
        it('returns an array of all supported os themes', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.supportedOsThemes).to.be.a('Array');
            chai.expect(sut.supportedOsThemes).to.have.lengthOf(1);
            chai.expect(sut.supportedOsThemes).to.include.members(['mac']);
        });
    });
    ...
    describe('get defaultOsTheme', () => {
        it('returns a supported os theme', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.defaultOsTheme).to.be.oneOf(sut.supportedOsThemes);
        });

        it('returns mac', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.defaultOsTheme).to.be.equal('mac');
        });
    });
    ...
    describe('get windowState', () => {
        it('returns the default windowState when the attribute window-state is null', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.windowState).to.equal(sut.defaultWindowState);
        });

        it('returns the value of the attribute window-state for supported window states', () => {
            const sut = document.createElement('os-window');
            for(let i = 0; i < sut.supportedWindowStates.length; i++) {
                sut.setAttribute('window-state', sut.supportedWindowStates[i]);
                chai.expect(sut.windowState).to.equal(sut.supportedWindowStates[i]);
            }
        });
    ...
    describe('set windowState', () => {
        ...
        it('accepts all supported window states', () => {
            ...
            try {
                for(let i = 0; i < sut.supportedWindowStates.length; i++) {
                    sut.windowState = sut.supportedWindowStates[i];
                }
            } catch (e) {
                chai.expect.fail(e);
            }
    ...
    describe('get defaultWindowState', () => {
        it('returns a supported window state', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.defaultWindowState).to.be.oneOf(sut.supportedWindowStates);
        });

        it('returns maximized', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.defaultWindowState).to.be.equal('maximized');
        });
    });
    ...
    describe('get supportedWindowStates', () => {
        it('returns an array of all supported window states', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.supportedWindowStates).to.be.a('Array');
            chai.expect(sut.supportedWindowStates).to.have.lengthOf(2);
            chai.expect(sut.supportedWindowStates).to.include.members(['maximized', 'minimized']);
        });
    });
    ...
    describe('get windowTitle', () => {
        it('returns the default window title when the attribute windowTitle is null', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.windowTitle).to.equal(sut.defaultWindowTitle);
        });
    ...
    describe('get defaultWindowTitle', () => {
        it('returns an empty string', () => {
            const sut = document.createElement('os-window');
            chai.expect(sut.defaultWindowTitle).to.equal('');
        });
    });
});
